...
import { updateEmployee, uploadEmployeeAvatar } from '../../api/employees.api.js';
...
export default function EditEmployeeDrawer({ open, onClose, employee, isAdmin }) {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: async (data) => {
      ...
      else if (avatarCleared) payload.avatarUrl = null;
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ['employee', String(employee.id)] });
      qc.invalidateQueries({ queryKey: ['employees'] });
      onClose();
    },
    onError: (err) => toast.error(err.response?.data?.error || 'Failed to update employee'),
  });

  return (
    <Drawer open={open} onClose={onClose} title="Edit Profile" subtitle="Update employee information" width="460px">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2">
              <Field label="New Password (optional)" error={errors.password?.message}>
                <input style={fieldStyle} type="password" placeholder="Leave empty to keep current"
                  {...register('password', { minLength: { value: 8, message: 'Min 8 chars' } })} />
              </Field>
            </div>
            <Field label="Role *">
              <select style={fieldStyle} {...register('role')} disabled={!isAdmin}>
