...
export default function LeadStatusesPage() {
  ...
  const { mutate: doCreate, isPending: creating } = useMutation({
    mutationFn: () => createLeadStatus({ name: newName }),
    onSuccess: () => { setNewName(''); qc.invalidateQueries({ queryKey: ['lead-statuses'] }); toast.success('Status created'); },
  });

  const { mutate: doUpdate } = useMutation({
    mutationFn: (id) => updateLeadStatus(id, { name: editName }),
    onSuccess: () => { setEditId(null); qc.invalidateQueries({ queryKey: ['lead-statuses'] }); },
  });

  return (
    <div className="max-w-md">
      <h1 className="text-[16px] font-semibold mb-5" style={{ color: 'var(--text-primary)' }}>Lead Statuses</h1>
      ...
      <div className="flex gap-2">
        ...
        />
        <Button onClick={() => newName.trim() && doCreate()} disabled={creating || !newName.trim()} size="sm">
          <Plus size={12} /> Add
        </Button>
      </div>
    </div>
