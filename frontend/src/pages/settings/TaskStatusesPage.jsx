...
function StatusDrawer({ open, onClose, editItem }) {
  ...
  const { mutate: doCreate, isPending: creating } = useMutation({
    mutationFn: (data) => createTaskStatus(data),
    onSuccess: () => {
  ...
  const { mutate: doUpdate, isPending: updating } = useMutation({
    mutationFn: (data) => updateTaskStatus(editItem.id, data),
    onSuccess: () => {
  ...
  return (
    <Drawer open={open} onClose={handleClose}
      title={isEdit ? 'Edit Task Status' : 'New Task Status'}
      subtitle={isEdit ? `Editing "${editItem?.name}"` : 'Add a new workflow stage'}
      width="380px">
      <form onSubmit={handleSubmit} className="space-y-4">
        <Field label="Status Name *">
          <input style={fieldStyle} placeholder="e.g. QA Testing" value={name} onChange={e => setName(e.target.value)} autoFocus />
        </Field>
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          <h1 className="text-[16px] font-semibold" style={{ color: 'var(--text-primary)' }}>Task Statuses</h1>
          <p className="text-[12px] mt-0.5" style={{ color: 'var(--text-tertiary)' }}>
            {statuses?.length || 0} workflow stages
          </p>
        </div>
      ...
      {statuses?.length > 0 ? (
        ...
        <div className="rounded-xl px-6 py-14 text-center"
          ...
          <p className="text-[13px] font-medium" style={{ color: 'var(--text-tertiary)' }}>No task statuses yet</p>
